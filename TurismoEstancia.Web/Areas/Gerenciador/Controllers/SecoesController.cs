...
public class SecoesController : PainelController
{
    ...
    {
        ViewData["Title"] = "Mapa";
        var vm = await MontarAsync("mapa", "Mapa interativo", "Imagens do município (PNG desktop + mobile) + chamada e pontos. Os pins usam % do container e se adaptam.", ct,
            textos: [
            ...
            imagens: [
                I("mapa-imagem", "Imagem do mapa — desktop (PNG, 1600×900, até 1 MB)"),
                I("mapa-imagem-mobile", "Imagem do mapa — mobile (PNG vertical 800×1200, opcional; se vazia usa a desktop)")
            ],
            ancora: "#section-mapa");
        ...
        vm.MapaImagemZoomMobile = dic.TryGetValue("mapa-imagem-zoom-mobile", out var zm) && int.TryParse(zm, out var zim) && zim is >= 100 and <= 250 ? zim : 100;
        vm.MapaImagemPosXMobile = dic.TryGetValue("mapa-imagem-pos-x-mobile", out var xm) && int.TryParse(xm, out var xim) && xim is >= 0 and <= 100 ? xim : 50;
        vm.MapaImagemPosYMobile = dic.TryGetValue("mapa-imagem-pos-y-mobile", out var ym) && int.TryParse(ym, out var yim) && yim is >= 0 and <= 100 ? yim : 50;
