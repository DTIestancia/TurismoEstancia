        ...
        IReadOnlyList<PontoTuristicoDto> pontos)
    {
        ...
        foreach (var p in pontos.Where(x => x.ExibirNoMapa && x.Ativo).OrderBy(x => x.Ordem))
        {
            ...
            pois.Add(new
            {
                ...
                desc = p.Descricao,
                detail = p.Detalhe,
                img = p.CapaArquivoId is long capaId ? Url.Content($"~/arquivo/{capaId}?largura=600") : "",
                icon = p.Icone ?? "map-pin",
                iconImg = (p.IconeArquivoId ?? cat.IconeArquivoId) is long iconeId ? Url.Content($"~/arquivo/{iconeId}?largura=160") : "",
                tag = p.Tag,
                address = p.Endereco,
